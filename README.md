...
> 💡 常驻模式下，行情按时间窗推送（上午 `start_time`～11:30 + 下午 13:00～`end_time`），收盘后自动停止当日推送，服务继续运行等待次日；`stock_push.event_notify` 开启时还会在启动 / 午间休市 / 下午开盘 / 收盘 / 退出时推送温馨提醒。
...
| `stock_push.event_notify` | 生命周期通知开关（默认 `true`）：启动成功 / 午间休市(11:30) / 下午开盘(13:00) / 收盘(15:00) / 退出前各推送一条温馨提示 |
